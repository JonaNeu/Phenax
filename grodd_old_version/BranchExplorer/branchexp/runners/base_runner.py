...
from branchexp.config import SEEN_TAGS_FILE, BLARE_FILE
from branchexp.explo.logcatctl import Controller


class RunInfos(object):
    """ Container class for Runner infos. """
...
class Runner(object):
    ...
    def __init__(self, infos):
        ...
        Args:
            infos: a RunInfos object with appropriate data for this run.
        """
        self.apk = infos.apk
        self.manifest = infos.manifest
        self.entry_point = infos.entry_point
        self.tools = infos.tools
        self.device = infos.device
        self.output_dir = infos.output_dir
    ...
    def start_logcat(self):
        ...
        log.info("LogcatController is running.")

    def stop_logcat(self):
        """ Stop the logcat subprocess. """
        self.logcatctl.stop_logcat()
        self.logcatctl.save_capture(self.capture_file)
        self.logcatctl = None
    ...
    def trace_apk(self, pkg = None):
        ...
        if data_dir is None:
            log.warning("Could not find data directory")
        if apk_location is None:
            log.warning("Could not find the APK location")
