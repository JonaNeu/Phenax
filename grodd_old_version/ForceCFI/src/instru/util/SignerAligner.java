...
public class SignerAligner {
	...
	public static void sign(String apk) {
		...
		try {
			Process p = pb.start();
			if (p.waitFor() == 0)
				System.out.println(apk + " signed with jarsigner.");
			else
				System.out.println("An error occured with jarsigner (is it in your PATH?),"
					+ " check .jarsigner_log.");
		} catch (IOException e) {
			e.printStackTrace();
			return;
	...
	public static void align(String apk) {
		...
		try {
			Process p = pb.start();
			if (p.waitFor() == 0)
				System.out.println(apk + " aligned with zipalign.");
			else
				System.out.println("An error occured with zipalign (is it in your PATH?),"
					+ " check .zipalign_log.");
		} catch (IOException e) {
			e.printStackTrace();
			return;
