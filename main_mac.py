import os, subprocess, argparse, glob, time, shutil, pickle
from Config_handler import Config_handler
from Utils import *
from MLA.extract_numerical_features import *
from MLA.Learn import predictKFoldSVM, predictAndTestKFoldSVM, predictKFoldSVMSSK, calculateMetrics, get_indices_of_misclassified_apps, predictKFoldTrees, predictKFoldKNN, predictKFoldSVM
from shutil import copyfile

# location to set the number of cores /usr/local/lib/python3.6/site-packages/acfg_tools/builder/app_graph.py

# Run command
# python3 main2.py -explorer_dir "/Users/jonaneumeier/Dropbox/Uni/6. Semester/Bachelor-thesis/bachelor_code/grodd_new_version/BranchExplorer"  -inputDir /Users/jonaneumeier/Dropbox/Uni/6.\ Semester/Bachelor-thesis/bachelor_code/test_data/input -outputDir /Users/jonaneumeier/Dropbox/Uni/6.\ Semester/Bachelor-thesis/bachelor_code/test_data/output

# prettyPrint color options: info, error, warning, info2, output, debug

def setup_Arguments():
    parser = argparse.ArgumentParser(prog="main.py", description="This is the main program. TODO: explain the program here briefly")
    parser.add_argument("-explorer_dir", "--branchExplorer_dir", help="The path to the subdirectory BranchExplorer in the GroddDroid files", required=True)
    parser.add_argument("-inputDir", "--inputDirectory", help="The path to the directory with the input APKs. It should contain two directories train and test, each of them should ahve two folder malware and goodware as well", required=True)
    parser.add_argument("-outputDir", "--outputDirectory", help="The path to the directory for the output", required=True)
    parser.add_argument("-kf", "--kfold", help="Set the value of K for cross validation", default=2)
    parser.add_argument("-selectBest", "--selectBest", help="Set the number of the features that should be selected >= 30", default=0)
    parser.add_argument("-VMname", "--vm_name", help="The name or ID of the Virtual Box virtual machine to use", default="ad41ff7d-6a07-4a41-892f-efafe6b321ae")
    parser.add_argument("-restore_snapshot", "--restore_snapshot", help="The name or ID of the Virtual Box snapshot to restore the virtual machine", default="5bcea49c-8824-48c8-ae17-8885bb9f9d86")
    return parser
...
def main():
    ...
    f.write("# kFold: %s, Select K Best Features: %s #\n" % (str(arguments.kfold), str(arguments.selectBest)))
    f.write("############################################################################\n")
    f.close()

    f = open(arguments.outputDirectory + "/ml_input.txt", "a")
    f.write("############################################################################\n")
    f.write("# INSTRUCTIONS: Each iterations has 3 inputs. First all (hybrid) features. Second all static features and third all dynamic features #")
    f.write("############################################################################\n")
    f.close()
    ...
    for input_folders in input_dir:
        ...
        if len(allAPKs) < 1:
            prettyPrint("Could not find any APK's under \"%s/%s\". Exiting" % (arguments.inputDirectory, app_type), "error")
            return False

        prettyPrint("Successfully retrieved %s APK's from \"%s/%s\"" % (len(allAPKs), arguments.inputDirectory, app_type), "info2")
        ...
        for apk in allAPKs:
            ...
            restore_and_start_vm(arguments.vm_name, arguments.restore_snapshot)
    ...
    predicted, metrics, misclassified_apps = do_the_machine_learning(X_dynamic, y,
        arguments.outputDirectory + "/results.txt", arguments.selectBest, arguments.kfold, 0,
    ...
    while(run):
        ...
        # dynamic features - iterate depending on the metrics of these
        predicted, metrics, misclassified_apps = do_the_machine_learning(X_dynamic, y,
            arguments.outputDirectory + "/results.txt", arguments.selectBest, arguments.kfold, feedback_run_counter,
            arguments.outputDirectory + "/ml_input.txt", 3)
...
def extract_features_from_files(traces_file_path, targets_file_path, to_force_file_path, stats_file_path, manifest_file_path):
...
def do_the_machine_learning(X, y, output_dir, selectBest, kfold, iteration, output2_dir=None, feature_type=1):
    ...
    for k in K:
        ...
        missclassifiedDict["KNN%s" % k] = {"missclassified": tempMissclassified, "#crashed_apps": len(tempMissclassified)}
    ...
    for e in E:
        ...
        predicted = predictKFoldTrees(X, y, kfold=int(kfold), selectKBest=int(selectBest), estimators=e)
        ...
        tempMissclassified = get_indices_of_misclassified_apps(y, predicted)
        missclassifiedDict["Trees%s" % e] = {"missclassified": tempMissclassified, "#crashed_apps" : len(tempMissclassified)}
    ...
    for m in metricsDict:
        ...
        metricsDict[m]["precision"], metricsDict[m]["f1score"], str(missclassifiedDict[m]["missclassified"]),
...
def restore_and_start_vm(vm_id, snapshot_name):
    ...
    result = subprocess.Popen(args, stderr=subprocess.STDOUT, stdout=subprocess.PIPE).communicate()[0]
...
def shutdown_vm(vm_id):
    ...
    # args = ["/opt/genymobile/genymotion/player", "--poweroff", "--vm-name", arguments.vmname]
