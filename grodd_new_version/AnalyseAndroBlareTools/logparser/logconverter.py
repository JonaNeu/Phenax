...
argparser.add_argument('-i', help='Specify the input file to be parsed', action='store', required=True, dest='input')
...
argparser.add_argument('--o_type', help='Specify the file output type(s). Supported types are dot, gexf and pickle', required=True, nargs='+')
...
if os.path.isfile(args.input):
    filenamelist = [os.path.abspath(args.input)]
elif os.path.isdir(args.input):
    for root, d_names, fnames in  os.walk(args.input) :
        for fname in fnames:
            if (fname.endswith("-log.txt") or fname.endswith(".txt") or fname.endswith(".log")):
                filenamelist.append(os.path.abspath(os.path.join(root, fname)))
else:
    print "Provide a file or a directory\n"
    quit()

pool = Pool(processes=args.job)
sfglist = pool.map(buildsfg, filenamelist)

# Code commented as we know use multiprocessing
#sfglist = []
#for fname in filenamelist :
#    print("Start parsing " + fname + "\n")
#    res = buildsfg(fname)
#    print("SFG built for : " + fname + "\n")
#    sfglist.append((res, fname))

for sfg, fname in sfglist :
    if (sfg is None) :
        print("SFG null " + fname + "\n")
        continue
    oname = string.replace(string.replace(string.replace(fname, "-log.txt", ""), ".txt", ""), ".log", "")
    for file_format in args.o_type:
        if (file_format == "gml"):
            nx.write_gml(sfg, oname + ".gml")
            print("Graph stored in " + oname + ".gml")
        elif (file_format == "dot"):
            sfgdot = graphtodot.build_dot_graph(sfg, args.cluster_system)
            graphtodot.save_dot_to_file(sfgdot, oname + ".dot")
        elif (file_format == "gpickle"):
            nx.write_gpickle(sfg, oname + ".gpickle")
...
lineno = 1
# Build the system flow graph based on the log given in input
logfile = open(args.input)
previous_line = "" 
for line in logfile:
    new_line = clean_entry(line, regexp_list, discard_filter)
    if (not (new_line is None)) and (new_line != previous_line) and (len(new_line) > 5) :
        ...
        previous_line = new_line
    lineno += 1
print "Parsing duration: " + str(time.clock() - start_t)
print "Number of lines : " + str(lineno)

to_be_removed = []
for node in flow_graph.nodes() :
    if ((len(lsfg.get_out_edges(flow_graph, node)) == 0) and (len(lsfg.get_in_edges(flow_graph, node)) == 0)):
        to_be_removed.append(node)

for elt in to_be_removed:
    flow_graph.remove_node(elt)

if (len(flow_graph.nodes()) == 0):
    print "The correspondig system flow graph is empty"
    quit() 

o_filename = string.replace(string.replace(args.input, "-log.txt", ""), ".txt", "")
if (args.info_id != False):
    o_filename += ("-" + str(args.info_id))
