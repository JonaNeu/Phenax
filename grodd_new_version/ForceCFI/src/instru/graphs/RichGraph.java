...
//import soot.util.cfgcmd.ImplicitFlows;
...
public class RichGraph {
	 ...
	 */
	public <N> DotGraph drawCFG(DirectedGraph<N> graph, final Body body) {
		...
		for (Iterator<N> nodesIt = graph.iterator(); nodesIt.hasNext();) {
			N node = nodesIt.next();
			final DotGraphNode graphNode = canvas.drawNode(namer.getName(node));

			// Adrien (Shgck) additions : add stuff to graphs as attributes
			graphNode.setAttribute(new DotGraphAttribute("method", Integer.toString(body.getMethod().getNumber())));

			if (node instanceof Unit) {
				Unit unit = (Unit) node;

				graphNode.setAttribute(new DotGraphAttribute("key", Integer.toString(unit.hashCode())));

				// invoke_target
				unit.apply(new AbstractStmtSwitch() {
					public void caseInvokeStmt(InvokeStmt invoke) {
						processInvoke(invoke.getInvokeExpr());
					}

					public void caseAssignStmt(AssignStmt stmt) {
						// Looking just for Invoke statements on the right side
						// of the assignment
						if (stmt.containsInvokeExpr()) {
							// if it is an invoke statement
							processInvoke(stmt.getInvokeExpr());
						}
					}

					private void processInvoke(InvokeExpr expr) {
						// if(expr.getMethodRef().getSignature().contains("android.support"))
						if (Main.is_class_useless(expr.getMethodRef().declaringClass().toString())
						...
						for (Value v : expr.getArgs()) {
							realParams.add(v.getType().toString());
						}
						
						MethodSignature realMethSig = new MethodSignature(
								expr.getMethodRef().declaringClass().toString(),
								expr.getMethodRef().returnType().toString(), expr.getMethodRef().name(), realParams,
								body.getMethod().toString());
						
						// Hook Sup classes and interfaces
						if (Main.doImplicit) {
							ImpGen.v().addInvokeSignature(realMethSig);
							ImpGen.v().setSup(expr.getMethodRef().declaringClass());
							ImpGen.v().setSup(expr.getMethod().getDeclaringClass());
							ImpGen.v().setSup(Scene.v().getSootClass(expr.getMethod().getReturnType().toString()));
							ImpGen.v().setSup(Scene.v().getSootClass(expr.getMethodRef().returnType().toString()));
							ImpGen.v().setSup(Scene.v().getSootClass(expr.getType().toString()));
							for (String p : realParams) {
								ImpGen.v().setSup(Scene.v().getSootClass(p));
							}
							for (soot.Type paramType : expr.getMethod().getParameterTypes()) {
								SootClass paramClass = Scene.v().getSootClass(paramType.toString());
								ImpGen.v().setSup(paramClass);
							}
						}
						// End Hook

						// Add the signature of the target method, useful to
						// determine objects types
						String targetSig = "\"" + expr.getMethodRef().getSignature() + "\"";
						graphNode.setAttribute(new DotGraphAttribute("invoke_target", targetSig));
						graphNode.setAttribute(
								new DotGraphAttribute("invoke_real", "\"<" + realMethSig.toString() + ">\""));

					}
				});

				// ImplicitFlows.handle(unit, body, graphNode);
			}
			// end of addition
	...
	public <N> DotGraph drawCFG(ExceptionalGraph<N> graph) {
		...
		for (Iterator<N> nodesIt = graph.iterator(); nodesIt.hasNext();) {
			...
			for (Iterator<N> succsIt = sortedIterator(graph.getUnexceptionalSuccsOf(node), nodeComparator); succsIt
					.hasNext();) {
				N succ = succsIt.next();
			...
			for (Iterator<N> succsIt = sortedIterator(graph.getExceptionalSuccsOf(node), nodeComparator); succsIt
					.hasNext();) {
				N succ = succsIt.next();
				DotGraphEdge edge = canvas.drawEdge(namer.getName(node), namer.getName(succ));
			...
			if (showExceptions) {
				for (Iterator<ExceptionDest<N>> destsIt = sortedIterator(graph.getExceptionDests(node),
						new ExceptionDestComparator(namer)); destsIt.hasNext();) {
